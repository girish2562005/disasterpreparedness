-- Fix quiz security vulnerability by implementing role-based access control

-- Drop the existing policy that exposes answers to everyone
DROP POLICY IF EXISTS "Everyone can view quiz questions" ON public.quiz_questions;

-- Create role-aware policies for quiz questions
CREATE POLICY "Students can view questions without answers" 
ON public.quiz_questions 
FOR SELECT 
USING (
  get_current_user_role() = 'student' OR 
  get_current_user_role() IS NULL
);

CREATE POLICY "Teachers and admins can view complete questions" 
ON public.quiz_questions 
FOR SELECT 
USING (get_current_user_role() = ANY (ARRAY['admin', 'teacher']));

-- Create a secure function to validate quiz answers
...
-- Create a function to submit complete quiz and get score
